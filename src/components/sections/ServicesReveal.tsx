...
export default function ServicesReveal() {
  ...
  return (
    ...
    >
      {/* ========================================================================= */}
      {/* DESKTOP / TABLET: Full Sticky Immersive Scroll Reveal (450vh)             */}
      {/* ========================================================================= */}
      <div className="hidden md:block relative w-full h-[450vh]">
        {/* Sticky Full-Viewport Stage */}
        <div className="sticky top-0 h-screen min-h-[100dvh] w-full overflow-hidden flex flex-col justify-between px-8 lg:px-16 py-10 lg:py-14 bg-black">
          {/* Spatial Depth & Energy Core Background (Continues the final frame of hero video) */}
          <div className="absolute inset-0 pointer-events-none bg-[radial-gradient(circle_at_50%_35%,rgba(56,189,248,0.12)_0%,rgba(99,102,241,0.06)_35%,transparent_70%)]" />
          {/* Subtle tunnel floor perspective grid lines */}
          <div className="absolute inset-0 pointer-events-none bg-[linear-gradient(to_right,rgba(255,255,255,0.02)_1px,transparent_1px),linear-gradient(to_bottom,rgba(255,255,255,0.02)_1px,transparent_1px)] bg-[size:64px_64px] opacity-40 [mask-image:radial-gradient(ellipse_60%_50%_at_50%_50%,#000_70%,transparent_100%)]" />

          {/* Top Stage Header */}
          <div className="relative z-20 flex items-end justify-between border-b border-white/10 pb-6">
            <div>
              <div className="flex items-center gap-3">
                <span className="h-px w-6 bg-cyan-400" />
                <span className="text-xs font-mono tracking-[0.3em] uppercase text-cyan-400 font-medium">
                  WHAT ADMAKI BUILDS
                </span>
              </div>
              <h2 className="mt-2 text-2xl lg:text-3xl font-black uppercase tracking-tight text-white">
                From Ideas <span className="text-zinc-500 font-light">—</span> To Working Systems.
              </h2>
            </div>
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-baseline gap-1 font-mono text-sm">
                <span ref={currentNumRef} className="text-white font-bold text-lg">01</span>
                <span className="text-zinc-600">/</span>
                <span className="text-zinc-500 text-xs">04</span>
              </div>
              <div className="w-28 lg:w-36 h-1 bg-white/10 rounded-full overflow-hidden">
                <div
                  ref={progressBarRef}
                  className="h-full bg-gradient-to-r from-cyan-400 to-indigo-500 rounded-full transition-all duration-75"
                  style={{ width: '4%' }}
                />
              </div>
            </div>
              ...
              >
                ...
                <div className="w-1/2 flex flex-col justify-center pr-6">
                  ...
                  <div className="flex items-center gap-3 mb-2">
                    <span className="text-5xl lg:text-6xl font-mono font-extrabold tracking-tighter text-cyan-400/90">
                      {svc.num}
                    </span>
                  ...
                  <h3 className="text-4xl lg:text-6xl xl:text-7xl font-black uppercase tracking-[-0.03em] leading-[0.92] text-white">
                  ...
                  {/* Short Description */}
                  <p className="mt-5 text-sm lg:text-base text-zinc-400 font-light leading-relaxed max-w-lg">
                    {svc.description}
                  </p>
                  ...
                  <div className="mt-7 flex flex-wrap items-center gap-2">
                    {svc.capabilities.map((cap) => (
                      <span
                        key={cap}
                        className="px-3.5 py-1.5 rounded-full border border-white/15 bg-white/[0.04] text-xs font-mono text-zinc-300 backdrop-blur-md"
                      >
                        {cap}
                      </span>
                    ))}
          ...
          <div className="relative z-20 flex items-center justify-between border-t border-white/10 pt-4 text-xs font-mono text-zinc-500">
            <span className="hidden lg:inline">
              Creative, technical, and business-focused digital solutions built around real needs.
            </span>
            <span className="text-cyan-400/80 uppercase tracking-widest text-[10px]">
              Scroll to scrub through capabilities ↓
            </span>
          </div>
        </div>
      </div>
      ...
      {/* ========================================================================= */}
      <div className="md:hidden w-full px-6 py-16 space-y-16">
        {/* Mobile Header Intro */}
        <div className="border-b border-white/10 pb-8">
          <div className="flex items-center gap-2 mb-3">
            <span className="h-px w-5 bg-cyan-400" />
            <span className="text-[10px] font-mono tracking-[0.25em] uppercase text-cyan-400">
              WHAT ADMAKI BUILDS
            </span>
          </div>
        ...
        <div className="space-y-12">
          {servicesData.map((svc, i) => (
            <div
              key={svc.id}
              className="rounded-2xl border border-white/10 bg-zinc-950/80 p-6 space-y-6 shadow-2xl"
            >
              {/* Card Header */}
              <div className="flex items-center justify-between border-b border-white/10 pb-4">
                <span className="text-3xl font-mono font-black text-cyan-400">{svc.num}</span>
              ...
              <div className="flex flex-wrap gap-1.5 pt-2">
                {svc.capabilities.map((cap) => (
                  <span
                    key={cap}
                    className="px-2.5 py-1 rounded-full border border-white/15 bg-white/[0.04] text-[10px] font-mono text-zinc-300"
                  >
                    {cap}
                  </span>
                ))}
